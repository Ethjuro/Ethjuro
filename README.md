<!---- Animated greeting & short tagline ---->
<h1 align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?font=Inter&size=48&center=true&vCenter=true&width=600&height=70&color=FF4B4B&duration=4000&lines=Hi+there!+I’m+Ethjurø" alt="Typing SVG">
</h1>
<h3 align="center">Full-Stack Developer · Genève · Transforming ideas into code</h3>

<!---- Skill badges ---->
<br>
<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,bash,php,cs,sqlite,js,ts,react,vue,nodejs,laravel,dotnet,mongodb,debian,mysql,docker,sass,git,cloudflare&perline=10" alt="Skills" />
</p>

<!---- Contact / Social links ---->
<p align="center">
  <img src="https://img.shields.io/badge/Gmail-333333?style=for-the-badge&logo=gmail&logoColor=red&link=mailto%3Aethjuro%40ethjuro.com" alt="Gmail">
  <img src="https://img.shields.io/badge/Discord-383e9c?style=for-the-badge&logo=discord&link=https%3A%2F%2Fdiscord.com%2Fusers%2F672521122562048051" alt="Discord">
  <img src="https://img.shields.io/badge/Portfolio-9c3838?style=for-the-badge&link=https%3A%2F%2Fethjuro.com" alt="Portfolio">
</p>

<!---- GitHub stats ---->
<br>
<div align="center">
  <img width="420" src="https://github-readme-stats.vercel.app/api?username=ethjuro&theme=transparent&count_private=true&show_icons=true" alt="GitHub Stats">
  <img width="445" src="https://streak-stats.demolab.com?user=ethjuro&theme=dark&locale=fr&date_format=j%2Fn%5B%2FY%5D" alt="GitHub Streak"/>
</div>

<hr>

### 🎯 What I’m doing now  
- Learning advanced system design & scalable web architecture  
- Building performant full-stack applications  
- Exploring real-time dashboards for game server ecosystems

<!---- Optional visitor counter ---->
<hr>
<div align="center">
  <p>Visitor count</p>
  <img src="https://profile-counter.deno.dev/ethjuro/count.svg" alt="Visitor's Count" />
</div>
